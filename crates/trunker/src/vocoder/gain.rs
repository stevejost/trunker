...
impl Gains {
    ...
    pub fn new(gain_index: usize, amplitudes: &QuantizedAmplitudes, params: &BaseParams) -> Self {
        ...
        self.0[0]
            + 2.0
                * (2..=6).fold(0.0f32, |sum, m| {
                    sum + self.0[m - 1] * (PI * (m as f32 - 1.0) * (i as f32 - 0.5) / 6.0).cos()
                })
    }
}
