...
use trunker::decode::ControlChannelConfig;
use trunker::decode::TrunkedDecoderConfig;
use trunker::decode::control_channel;
use trunker::decode::heartbeat;
use trunker::decode::trunked;
...
fn main() -> Result<()> {
    ...
    match cli.command {
        Command::Cc {
            ...
            let cc_config = ControlChannelConfig {
                sample_rate,
                offset_hz,
                modulation: pipeline_modulation,
                nid_integrity: nid_policy,
                decode_audio,
                audio_file,
                heartbeat_seconds,
            };
            control_channel::decode_control_channel(&mut sample_source, &cc_config, &running)?;
        ...
        Command::Trunk {
            ...
            let trunk_config = TrunkedDecoderConfig {
                sample_rate,
                ...
                call_timeout,
                decode_audio,
                output_dir: output_dir.map(|d| Path::new(&d).to_path_buf()),
